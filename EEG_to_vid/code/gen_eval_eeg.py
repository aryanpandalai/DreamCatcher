...
if __name__ == '__main__':
    ...
    root = args.root  # e.g. '/content/DreamDiffusion'
    target = args.dataset

    sd = torch.load(args.model_path, map_location='cpu')
    config = sd['config']
    
    # Update paths: make sure to override the EEG signals path too!
    config.root_path = root
    config.eeg_signals_path = os.path.join(config.root_path, 'datasets', 'eeg_5_95_std.pth')
    config.pretrain_mbm_path = os.path.join(config.root_path, 'pretrains','generation','checkpoint.pth')
    config.pretrain_gm_path = os.path.join(config.root_path, 'pretrains')
    
    # Optionally update the splits path as well
    ...
    length_test_data = len(dataset_test)
    print(f'length of test data {length_test_data}')
    ...
    grid, _ = generative_model.generate(dataset_train, config.num_samples, config.ddim_steps, config.HW, 25)
    grid_imgs = Image.fromarray(grid.astype(np.uint8))
    grid_imgs.save(os.path.join(output_path, 'samples_train.png'))
    
    grid, samples = generative_model.generate(
        dataset_test, config.num_samples, config.ddim_steps, config.HW,
        limit=25, state=None, output_path=output_path
    )
    grid_imgs = Image.fromarray(grid.astype(np.uint8))
    grid_imgs.save(os.path.join(output_path, 'samples_test.png'))
